...
Currently configured project: `mighty-strolling-llama` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/mighty-strolling-llama/SETUP.md` to fill in
   `data/mighty-strolling-llama/INFO.md`.
...
`--project-id mighty-strolling-llama` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/mighty-strolling-llama/`.
