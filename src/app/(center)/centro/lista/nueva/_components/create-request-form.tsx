...
/**
 * Crear lista (Figma 32:4929). Client-owned form state: donation items (filled
 * via the co-located selector sheet), instrucciones + counter. Submits the
 * real `publishLista` server action; the selector returns its picks into
 * state. (The "área" facet was dropped — categories are derived from the
 * chosen insumos server-side. No title, no time window — lista-model-v2 §3d.)
 */
export function CreateRequestForm({ supplies }: { supplies: Supply[] }) {
  ...
  const canSubmit = items.length > 0 && instructions.length <= INSTRUCTIONS_MAX;
