...
/** "Vence en 23 h" / "Vence en 45 min" — for a pending invite's 24 h TTL. */
export function formatInviteExpiry(
  ...
  if (ms < HOUR) return `Vence en ${Math.max(1, Math.round(ms / MIN))} min`;
  return `Vence en ${Math.round(ms / HOUR)} h`;
}
